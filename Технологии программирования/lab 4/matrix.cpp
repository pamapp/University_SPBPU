...
bool Matrix::isOverlap(const Shape *shape1, const Shape *shape2) const
{
  ...
  return ((fabs(shape1->getFrameRect().point.x - shape2->getFrameRect().point.x)
        < ((shape1->getFrameRect().width / 2) + (shape2->getFrameRect().width / 2)))
      && ((fabs(shape1->getFrameRect().point.y - shape2->getFrameRect().point.y)
