...
void Parser::statement()
{
	...
	// станет известным только после того, как будет сгенерирован код для блока THEN.
        ...
        for (i = 0; i < break_counter[in_while - 1]; ++i) {
            codegen_->emitAt(break_addr[in_while - 1][i], JUMP, codegen_->getCurrentAddress()); //записываем JUMP во все сохраненные адреса break_addr
        }
        ...
        if (in_while > 0) {
            if (break_counter[in_while - 1] < BREAK_ADDR_MAX_SIZE) {
                int conditionAddress = codegen_->getCurrentAddress();
                relation();
                break_addr[in_while - 1][break_counter[in_while - 1]++] = conditionAddress++;

                if (codegen_ != NULL) {
                    codegen_->emitAt(break_addr[in_while - 1][i], JUMP, codegen_->getCurrentAddress());
                }
            }
            else {
                reportError("too much breaks in while");
            }
        }
        else {
            reportError("break should be in loop");
        }
    }
