...
void Rectangle::scale(double factor)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    top_[i] = { rectangle_.pos.x + (top_[i].x - rectangle_.pos.x) * factor,
        rectangle_.pos.y + (top_[i].y - rectangle_.pos.y) * factor };
  }
...
void Rectangle::rotate(double angle)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    top_[i] = { recPos.x + (top_[i].x - recPos.x) * angleCos - (top_[i].y - recPos.y)
        * angleSin, recPos.y + (top_[i].y - recPos.y) * angleCos + (top_[i].x - recPos.x) * angleSin };
  }
}

point_t Rectangle::getTop(const int index) const
{
  if (index > 3)
  {
    throw std::invalid_argument("Invalid index of top!");
  }
  return top_[index];
}
